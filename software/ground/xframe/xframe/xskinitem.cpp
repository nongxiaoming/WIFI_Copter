...
XSkinItem::XSkinItem(QWidget *parent)
	:QWidget(parent)
{
...
void XSkinItem::translateLanguage()
{
...
void XSkinItem::changeSkin(QString pixmap_name, QString skin_name, QString download_count)
{
...
void XSkinItem::paintEvent(QPaintEvent *)
{
	if(mouse_enter)
	{
		//绘制边框 
...
void XSkinItem::mouseReleaseEvent(QMouseEvent *event)
{
    Q_UNUSED(event);
	mouse_press = false;
}

void XSkinItem::enterEvent(QEvent *event)
{
    Q_UNUSED(event);
	mouse_enter = true;
	update();
...
void XSkinItem::leaveEvent(QEvent *event)
{
    Q_UNUSED(event);
	mouse_enter = false;
	update();
