	...
	:QWidget(parent)
{
    ...
    close_button = new XPushButton(this);
	...
	for(int i = 0; i < WINDOW_BUTTON_COUNT; i++)
	{
        ...
        connect(label, SIGNAL(signalLabelPress(XLabel*)), this, SLOT(changeCurrentPage(XLabel*)));
		label_array[i] = label;
	}
...
void CharacterWidget::changeCurrentPage(XLabel *label)
{
	for(int i=0; i<WINDOW_BUTTON_COUNT; i++)
	{
